import { analyzeAndCheckGrammar, analyzeResume, checkGrammar } from '../services/resumeService';
...
export const processAnalyzeResume = async (req: Request, res: Response): Promise<void> => {
    try {
        const { resumeText } = req.body;
        if (!resumeText) {
            console.error('No text provided in the request body');
...
export const processCheckGrammar = async (req: Request, res: Response): Promise<void> => {
    try {
        const { resumeText } = req.body;
        if (!resumeText) {
            console.error('No text provided in the request body');
